...
<script>
 ...
 <main>
      <div class="main-header">Quản lý thông báo</div>
      <div class="main-info">
        <form id="notification-form" onsubmit="saveSettings(event)">
          <div class="notification-option">
            <label for="booking-notif">Thông báo đặt phòng</label>
            <div class="toggle-switch">
              <input type="checkbox" id="booking-notif" name="booking-notif" checked>
              <span class="slider"></span>
            </div>
          </div>
          <div class="notification-option">
            <label for="promo-notif">Thông báo khuyến mãi</label>
            <div class="toggle-switch">
              <input type="checkbox" id="promo-notif" name="promo-notif">
              <span class="slider"></span>
            </div>
          </div>
          <div class="notification-option">
            <label for="update-notif">Thông báo cập nhật hệ thống</label>
            <div class="toggle-switch">
              <input type="checkbox" id="update-notif" name="update-notif" checked>
              <span class="slider"></span>
            </div>
          </div>
          <div class="notification-option">
  ...
  <script>
    function saveSettings(event) {
      ...
      const bookingNotif = document.getElementById('booking-notif').checked;
      const promoNotif = document.getElementById('promo-notif').checked;
      const updateNotif = document.getElementById('update-notif').checked;
      const emailNotif = document.getElementById('email-notif').checked;

      // Hiển thị thông báo trạng thái
      let statusMessage = "Cài đặt thông báo đã được lưu:\n";
      statusMessage += `- Thông báo đặt phòng: ${bookingNotif ? 'Bật' : 'Tắt'}\n`;
      statusMessage += `- Thông báo khuyến mãi: ${promoNotif ? 'Bật' : 'Tắt'}\n`;
      statusMessage += `- Thông báo cập nhật hệ thống: ${updateNotif ? 'Bật' : 'Tắt'}\n`;
      statusMessage += `- Nhận thông báo qua email: ${emailNotif ? 'Bật' : 'Tắt'}`;
      
      alert(statusMessage);

      // Lưu trạng thái vào localStorage (giả lập lưu dữ liệu)
      localStorage.setItem('bookingNotif', bookingNotif);
      localStorage.setItem('promoNotif', promoNotif);
      localStorage.setItem('updateNotif', updateNotif);
      localStorage.setItem('emailNotif', emailNotif);
    }

    // Khôi phục trạng thái khi tải lại trang
    window.onload = function() {
      const bookingNotif = localStorage.getItem('bookingNotif') === 'true';
      const promoNotif = localStorage.getItem('promoNotif') === 'true';
      const updateNotif = localStorage.getItem('updateNotif') === 'true';
      const emailNotif = localStorage.getItem('emailNotif') === 'true';

      document.getElementById('booking-notif').checked = bookingNotif;
      document.getElementById('promo-notif').checked = promoNotif;
      document.getElementById('update-notif').checked = updateNotif;
      document.getElementById('email-notif').checked = emailNotif;
    };
  </script>
</body>
</html>